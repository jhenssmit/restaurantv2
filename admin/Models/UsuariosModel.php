...
class UsuariosModel extends Query{
    private $usuario, $nombre, $clave, $id_caja, $id, $estado, $img;
    ...
    public function getUsuariosInac()
    {
        $sql = "SELECT u.*, c.id as id_caja, c.caja FROM usuarios u INNER JOIN caja c WHERE u.id_caja = c.id AND u.estado = 0";
        $data = $this->selectAll($sql);
        return $data;
    }
    public function registrarUsuario(string $usuario, string $nombre, string $clave, int $id_caja,  string $img)
    {
        ...
        if (empty($existe)) {
            # code...
            $sql = "INSERT INTO usuarios(usuario, nombre, clave, id_caja, imagen) VALUES (?,?,?,?,?)";
            $datos = array($this->usuario, $this->nombre, $this->clave, $this->id_caja, $this->img);
    ...
    public function modificarUsuario(string $usuario, string $nombre, int $id_caja, string $img,int $id)
    {
        $this->usuario = $usuario;
        $this->nombre = $nombre;
        $this->img = $img;
        $this->id = $id;      
        $this->id_caja = $id_caja;
        $sql = "UPDATE usuarios SET usuario = ?, nombre = ?, id_caja = ?, imagen = ? WHERE id = ?";
        $datos = array($this->usuario, $this->nombre, $this->id_caja,  $this->img, $this->id);
        $data = $this->save($sql, $datos);
        if ($data == 1) {
            $res = "modificado";
        } else {
